...
Diseases such as obesity, diabetes, coronary heart disease, and musculoskeletal disorders have rapidly increased in the United States in the last decade, which can be partially attributed to the change in office culture to be heavily computer-based and therefore more sedentary. The purpose of our analysis is to answer what is affecting the weight change of employees in a call center.  Lifestyle habits of employees affect rates of obesity and obesity-related diseases in the United States. Descriptive statistics and graphs are used to visualize relationships between different categorical and continuous variables. We will confirm normality for these variables and use log transformation for right skewed distributions. Work shift and total metabolic minutes are two variables at the center of our analysis. We will show their effect on weight gain through exploratory data analysis. Two models have been built to show the statistical significance of certain predictors. The first model uses logistic regression with weight gain (yes/no) being the categorical response variable. Our second model is a multivariate linear model that focuses on the significance of shift start time and total metabolic minutes. The following report records our central ideas and provides evidence for the procedures taken. 
...
#decent
p1 <- ggplot(na.omit(pracdat), aes(x = shift, y = mean(VigTime), fill=gender)) + geom_bar(stat="identity") + theme_minimal() + theme(axis.text.x = element_text(angle=90, hjust=1)) + xlab("Shift Time") + ylab("Average Vig Time") 
p2 <- ggplot(na.omit(pracdat), aes(x = shift, y = mean(WalkTime), fill=gender)) + geom_bar(stat="identity") + theme_minimal() +theme(axis.text.x = element_text(angle=90, hjust=1))+ xlab("Shift Time") + ylab("Average Walk Time")
p3 <- ggplot(na.omit(pracdat), aes(x = shift, y = mean(ModTime), fill=gender)) + geom_bar(stat="identity") + theme_minimal() + theme(axis.text.x = element_text(angle=90, hjust=1))+ xlab("Shift Time") + ylab("Average Mod Time")
p4 <- ggplot(na.omit(pracdat), aes(x = shift, y = mean(Total_Met_Min), fill=gender)) + geom_bar(stat="identity") + theme_minimal() + theme(axis.text.x = element_text(angle=90, hjust=1))+ xlab("Shift Time") + ylab("Average Total Metabolic Minutes")
