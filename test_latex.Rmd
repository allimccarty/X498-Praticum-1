...
ggplot(data = pracdat.abnormal , aes(x = value)) + stat_density() + facet_wrap(~variable, scales = "free") +theme_bw()
...
ggplot(data = pracdat.abnormal , aes(x = (value^(1/3)))) + stat_density() + facet_wrap(~variable, scales = "free")+theme_bw()
...
filter(pracdat, !is.na(gender)) %>% ggplot(aes(x = Age, y = Total_Met_Min, color = gender)) + geom_point(size = .8)+geom_smooth(method="lm", se=FALSE) + facet_grid(. ~ gender) + theme_bw() + theme(panel.grid = element_blank())+labs(caption="Figure X")+ggtitle("Total Metabolic Minutes by Age and Gender")+ylab("Total Metabolic Minutes")
```

We also want to see if there is an association between age & BMI when we stratify for gender.  Figure X demonstrates that there is slightly positive relationship between age and BMI, and the relationship is more dramatic for females compared to males. We should also note that there are number of observations with 0-value inpute for total metabolic minutes.  0-value observations for metabolic minutes are more common for females than males, and tend to be observed most commonly in females under the age of 50 years.

```{r}
#Scatterplot between Age &BMI with gender
filter(pracdat, !is.na(gender)) %>% ggplot(aes(x=Age, y = BMI, color=gender))+ geom_point()+theme_bw()+geom_smooth(method="lm",se=FALSE)+ggtitle("Association between Age & BMI by Gender") + labs(caption="Figure X")
```
...
filter(pracdat, !is.na(gender)& !is.na(shift.recode)) %>% ggplot(aes(x= gender, y =BMI_change, color=gender)) + geom_boxplot() + facet_grid(. ~ shift)+theme_bw() +labs(caption="Figure X") + ggtitle("Association of BMI Change and Shift by Gender")+ theme(axis.text.x = element_text(angle=90, vjust=.5,hjust=1))
```
...
filter(pracdat, !is.na(gender) & !is.na(shift.recode)) %>% ggplot(aes(x= gender, y =BMI_change, color=gender)) + geom_boxplot() + facet_grid(. ~ shift.recode)+theme_bw()+ ggtitle("Association of BMI Change and Recoded Shift by Gender")+ theme(axis.text.x = element_text(angle=90, vjust=.5,hjust=1))
```


The difference is more prominant when we are using recoded shift rather than shift.  It appears that indivduals with the morning shift experienced a greater change in BMI than individuals with a shift time in the afternoon. Females with a shift time of other also tended to have a greater high-end change in BMI over the study, but there were so few females in this category that the resulting distribution is probably not generalizable. 
...
a2 <- ggplot(data=pracdat, aes(x=Total_Met_Min, y=pounds_gained, color=shift.recode))+geom_point()+theme_bw()+ggtitle("Pounds Gained and Total Metabolic Minutes by Recoded Shift") + labs(caption="Figure X")
grid.arrange(a1, a2)
```


Figure X demonstrates the associations of weight gain with age, height, BMI, and metabolic minutes. There does not appear to be any strong patterns for the binary variable weight gain with age, height, BMI, and metabolic miuntes.  Thus, we do not suspect that age, height, BMI, or metabolic minutes are strongly indicative of whether a person gained weight or not.
...
check4 <- filter(pracdat, !is.na(weightgain)) %>% ggplot()+
  geom_boxplot(mapping = aes(x = weightgain, y=Total_Met_Min))+coord_flip()+theme_bw()+ggtitle("Weight Gain by Total Metabolic Minutes") + labs(caption="Figure X")
...
```{r}
filter(pracdat, !is.na(gender)) %>% ggplot(aes(x=Total_Met_Min, y=pounds_gained, color=gender))+geom_point()+theme_bw()+geom_smooth(method="lm", se=FALSE)+ggtitle("Association between Pounds Gained and Total Metabolic Minutes by Gender")+labs(caption="Figure X")
```
